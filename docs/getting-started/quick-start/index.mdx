...
- **用户注册：** 后续注册的用户初始状态为**待审核**，需要管理员批准才能访问。
- **隐私和数据安全：** **所有数据**，包括登录详情，都**本地存储**在您的设备上。Open WebUI 确保**严格的保密性**和**无外部请求**，以增强隐私和安全性。
  - **所有模型默认为私有。** 模型必须通过群组显式共享或设为公开。如果模型分配给某个群组，只有该群组的成员才能看到它。如果模型设为公开，实例中的任何人都可以看到它。
    ...
    <Tabs>
      ...
      <TabItem value="development" label="开发">
        <div className='mt-5'>
          <h3>开发环境设置</h3>
          <p>
            对于想要贡献代码的开发者，请查看<a href="/getting-started/advanced-topics">高级主题</a>中的开发指南。
          </p>
        </div>
      </TabItem>
  ...
  <TabItem value="third-party" label="第三方">
    <Tabs>
      <TabItem value="pinokio-computer" label="Pinokio.computer">
        ...
        如需通过 Pinokio.computer 安装，请访问他们的网站：
...
- [http://localhost:3000](http://localhost:3000) 来访问 OpenWebUI。
- 或者在使用 Python 部署时访问 [http://localhost:8080/](http://localhost:8080/)。

现在您可以开始使用 OpenWebUI 了！

## 将 OpenWebUI 与 Ollama 一起使用
如果您正在将 OpenWebUI 与 Ollama 一起使用，请务必查看我们的 [Ollama 入门指南](/getting-started/quick-start/starting-with-ollama)，了解如何使用 Open WebUI 管理您的 Ollama 实例。
