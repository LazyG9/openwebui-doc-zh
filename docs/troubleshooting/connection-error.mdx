...
我们将帮助你顺利完成所有设置和运行。下面，你将找到针对不同场景的分步说明，以解决与 Ollama 和 Hugging Face 等外部服务器的常见连接问题。

## 🌟 连接到 Ollama 服务器

### 🚀 从 Open WebUI 访问 Ollama

从 Open WebUI 连接到 Ollama 时遇到困难？这可能是因为 Ollama 没有监听允许外部连接的网络接口。让我们来解决这个问题：

1. **配置 Ollama 以广泛监听** 🎧：
   将 `OLLAMA_HOST` 设置为 `0.0.0.0` 以使 Ollama 监听所有网络接口。

2. **更新环境变量**：
   确保在部署环境中正确设置了 `OLLAMA_HOST`。
...
如果在尝试访问 Ollama 时看到连接错误，可能是因为 WebUI docker 容器无法与主机上运行的 Ollama 服务器通信。让我们来修复这个问题：

1. **调整网络设置** 🛠️：
   在 Docker 命令中使用 `--network=host` 标志。这会将你的容器直接连接到主机的网络。
...
```
🔗 运行上述命令后，你的 WebUI 应该可以在 `http://localhost:8080` 访问。
